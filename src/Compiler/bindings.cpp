...
Bindings::Bindings()
	: break_label_(""), continue_label_(""), stack_counter_(0), expression_stack_(16), global_stack_counter_(-20)
{}
...
void Bindings::increaseGlobalStackPosition()
{
	global_stack_counter_ -= 4;
}

void Bindings::increaseGlobalStackPosition(const int &position)
{
	global_stack_counter_ -=position;
}
...
void Bindings::setFunName(const std::string &name)
{
	fun_name_ = name;
}
...
std::string Bindings::getFunName()
{
	return fun_name_;
}
...
int Bindings::currentGlobalStackPosition() const
{
	return global_stack_counter_;
}
